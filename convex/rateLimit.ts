...
/** Fixed-window rate limiter. One row per key; resets when the window elapses. Atomic via
 *  Convex's serializable mutations, so concurrent hits can't over-count. Used by the public
 *  API routes (bot / assemble / compat / voice) to stop cost-abuse and DoS. */
