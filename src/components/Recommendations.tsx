...
export function Recommendations({
  ...
  const recs = useQuery(api.catalog.recommendations, { slug, limit: 8 }) ?? [];
  const cart = useCart();
          ...
          return (
            <button
              ...
              className={`group relative flex flex-col overflow-hidden rounded-2xl text-left transition-all ${
                on
                  ? "glass ring-2 ring-accent-400"
                  : "glass glass-hover gradient-border"
              }`}
            >
              <span
                className={`absolute right-2 top-2 z-10 flex h-6 w-6 items-center justify-center rounded-full text-xs transition-colors ${
              ...
              <div className="flex flex-1 flex-col gap-1 p-3">
                ...
                <span className="mt-auto pt-1 text-sm text-accent-400">
                  ...
                  {q && days ? (
                    <span className="ml-1 text-xs text-white/40">· £{q.total} total</span>
                  ) : null}
                </span>
              </div>
            </button>
