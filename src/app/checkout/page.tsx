...
const DELIVERY_FEE = 25; // flat London delivery (placeholder — tune per SPEC)
const ms = (iso: string) => Date.parse(iso + "T00:00:00Z");

export default function CheckoutPage() {
  const { items, subtotal, depositTotal } = useCart();
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_320px]">
          ...
          <div className="flex flex-col gap-5">
            <div className="glass gradient-border rounded-2xl p-5">
              <h2 className="mb-3 font-display font-semibold text-white/80">
                Your details
              </h2>
              <div className="flex flex-col gap-3">
                ...
                <div className="flex gap-3">
                  <input
                    ...
                    placeholder="Name"
                    className="flex-1 rounded-lg bg-white/[0.04] px-4 py-2.5 text-sm text-white/80 outline-none placeholder:text-white/30"
                  />
                  <input
                    value={phone}
            ...
            <div className="glass gradient-border rounded-2xl p-5">
              ...
              <div className="flex gap-3">
                {(["pickup", "delivery"] as const).map((f) => (
                  <button
                    ...
                    className={`flex-1 rounded-lg px-4 py-2.5 text-sm capitalize transition-colors ${
                      fulfilment === f
              ...
              {fulfilment === "delivery" && (
                <textarea
                  ...
                  rows={3}
                  className="mt-3 w-full rounded-lg bg-white/[0.04] px-4 py-2.5 text-sm text-white/80 outline-none placeholder:text-white/30"
                />
              )}
            </div>
          </div>
            ...
            <div className="mt-4 flex flex-col gap-2 text-sm">
              {items.map((i) => (
                <div key={i.key} className="flex justify-between text-white/55">
                  <span className="mr-2 line-clamp-1">{i.title}</span>
