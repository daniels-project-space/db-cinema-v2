 ...
 * AND lens ranking. The bot API (src/app/api/bot/route.ts), the assembly UI
 * (src/app/api/assemble/route.ts) and the legacy gear.ts helper all funnel
 * through here so the "what mounts on what" rule can never drift again.
 *
...
/**
 * Backward-compatible boolean gate (legacy callers / gear.ts). A lens "fits" a
 * camera set unless the best compatibility is strictly incompatible.
